...
export class WebsocketMessageHandler {
    ...
    private getAllBlockChain(id: string): WsMessageDest {
        return {
            ...
            data: {
                id,
                blocks: this.blockchainService.getAllBlocks(id),
            },
    ...
    private getLatestBlock(id: string): WsMessageDest {
        return {
            ...
            data: {
                id,
                blocks: this.blockchainService.getLatestBlock(id),
            },
            dest: WsDestination.SINGLE,
        };
    }
    ...
    private async processBlockChain(
        data: Pick<Blockchain, 'id' | 'blocks'>,
    ): Promise<WsMessageDest> {
        try {
            await this.blockchainService.processBlockChain(data.blocks, data.id);
            return null;
        } catch (e) {
            if (e.toString().includes('Cannot process blockchain, additionnal info needed')) {
                return {
